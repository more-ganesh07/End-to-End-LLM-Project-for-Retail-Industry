-- Create the Database
CREATE DATABASE tshirts_retailer;
USE tshirts_retailer;
...
CREATE TABLE t_shirts (
    ...
    price INT CHECK (price BETWEEN 1000 AND 5000),
...
-- Create a store procesdure to populate the t_shirt table
DELIMITER $$
...
BEGIN
    ...
    WHILE counter < max_records DO
        ...
        SET price = FLOOR(1000 + RAND() * 4001);
        SET stock = FLOOR(10 + RAND() * 91);
        
        -- Attempt to insert a new record
        -- Duplicate brand, color, size combination will be ignored due to unique constraint
		BEGIN 
...
-- Call the stored procedure to populate the t_shirt table
CALL PopulateTShirts();

-- Insert at least 10 recort into the discount table
